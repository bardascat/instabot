...
class BotFollowersCrawler:
    ...
    def getUsersToScan(self):
        ...
        query = "select users.id_user, instagram_username,  email,(select date from instagram_user_followers where id_user=users.id_user order by date desc limit 1) as last_updated from users  join campaign on (users.id_user=campaign.id_user)  join user_subscription on (users.id_user = user_subscription.id_user)  where (user_subscription.end_date>now() or user_subscription.end_date is null) having (date(last_updated)<=DATE(CURDATE() - INTERVAL 3 DAY) or last_updated is null) order by -last_updated desc limit %s,%s"
